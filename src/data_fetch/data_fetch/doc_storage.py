...
class MongoDocumentStorage(DocumentStorage):
    ...
    async def insert(self, fetched_data: AsyncIterator[FetchedData]) -> None:
        ...
        while True:
            ...
            has_next, data_batch = await self.__get_data_batch(iter=fetched_data, size=self.DEFAULT_BATCH_SIZE)
    ...
    def __attempt_insert(self, data_batch: Iterable[FetchedData], number_of_attempts: int) -> None:
            src = "telegram" # TODO
            db = self.__client()[self.database_name]
            ...
            for attempt in range(1, 1 + number_of_attempts):
                try:
                    logger.debug("Inserting batch to the doc store. Attempt number {}/{}", attempt, number_of_attempts)
                    ...
                    if len_data_batch == len_inserted:
                        # success
                        logger.debug("Successfully inserted batch with {} documents", len_inserted)
                        break
                    else:
                        logger.warning("Batch insertion failed. Inserted {len_insert} of {len_batch} records", len_insert=len_inserted, len_batch=len_data_batch)

                    if attempt >= number_of_attempts:
                        # for now, just log the error
                        logger.error("Batch insertion failed after {} attempts", number_of_attempts)
    ...
    def __client(self) -> MongoClient[Mapping[str, Any]]:
        ...
        mongo_client = MongoClient(f"mongodb://{username}:{password}@{self.mongo_host}:{self.mongo_port}")  # type:ignore
        logger.debug("Mongo client object id: {}", id(mongo_client))  # TODO delete
        return mongo_client

    async def __get_data_batch(self, iter: AsyncIterator[FetchedData], size: int) -> Tuple[bool, Iterable[FetchedData]]:
        sentinel = object()
