...
async def main(
    ...
    maybe_key_store = AzureKeyStore(azure_keyvault_name) if azure_keyvault_name is not None else None

    data_fetcher_maybe = BaseDataFetcher.get_fetcher_by_arg(data_fetcher, maybe_key_store)
    ...
    last_inserted = doc_storage.get_last_inserted(data_fetcher_maybe.FETCHER_NAME)
    offset_date = (
        last_inserted.created_at if last_inserted is not None else DEFAULT_OFFSET_DATE
    )
    ...
    await doc_storage.insert(fetched_data=fetched_data_iterator_async, collection=data_fetcher_maybe.FETCHER_NAME)
