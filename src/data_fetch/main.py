...
@click.command()
@click.option("--data-fetcher", type=DataFetcherArg, required=True, help="Data fetcher")
...
@click.option("--azure-keyvault-name", type=str, help="Name of the azure keyvault from which to retrieve secrets", required=True)
@async_command
async def main(data_fetcher: DataFetcherArg, mongo_host: str, mongo_port: int, azure_keyvault_name: str) -> None:
    logger.info("Data fetch started")

    if not azure_keyvault_name:
        err_msg = "Azure keyvault name not provided"
        logger.error(err_msg)
        raise ValueError(err_msg)

    key_store = AzureKeyStore(azure_keyvault_name)

    data_fetcher_maybe = BaseDataFetcher.get_fetcher_by_arg(data_fetcher, key_store)
    doc_storage = MongoDocumentStorage(mongo_host, mongo_port, DATA_FETCH_DATABASE_NAME)

    if data_fetcher_maybe is None:
        not_registered_data_fetcher_err = f"Data fetcher {data_fetcher} not registered"
        logger.error(not_registered_data_fetcher_err)
        raise ValueError(not_registered_data_fetcher_err)

    last_inserted = doc_storage.get_last_inserted("telegram")
    offset_date = last_inserted.created_at if last_inserted is not None else DEFAULT_OFFSET_DATE
    ...
    await doc_storage.insert(fetched_data_iterator_async)
